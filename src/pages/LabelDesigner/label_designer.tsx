...
// Add this helper function for getting absolute position of nested containers
...
const PDFBarcodeDesigner: React.FC<PDFBarcodeDesignerProps> = ({
  ...
  // Modified: Allow containers inside containers
  const handleDropIntoContainer = (e: React.DragEvent<HTMLDivElement>, container: PlacedComponent) => {
    ...
    // Remove the check that prevented containers inside containers
    // Now containers CAN be dropped into other containers
    ...
    if (containerRect) {
      ...
      if (component) {
        ...
        const newComponent: PlacedComponent = {
          ...
          height: componentType === DesignerElementType.container ? 150 : 30,
          lineThickness: "1",
          lineColor: "#000000",
          lineType: "solid",
          lineWidth: 100,
        ...
        // Add container properties if it's a container
  ...
  const handleDrop = (e: React.DragEvent<HTMLDivElement>) => {
    ...
    if (canvasRect) {
      ...
      // Check if dropping into a container - Modified to find deepest container
      const placedComponents = templateData?.barcodeState?.placedComponents || [];
      let targetContainer: PlacedComponent | null = null;
      let maxDepth = -1;
      
      // Helper to calculate container depth
      const getContainerDepth = (comp: PlacedComponent): number => {
        let depth = 0;
        let currentId = comp.containerId;
        while (currentId) {
          depth++;
          const parent = placedComponents.find(c => c.id === currentId);
          currentId = parent?.containerId;
        }
        return depth;
      };
      
      // Find the deepest container at the drop position
      for (const comp of placedComponents) {
        if (comp.type === DesignerElementType.container) {
          // Calculate absolute position for nested containers
          const absolutePos = getAbsolutePosition(comp, placedComponents);
          ...
          if (
            ...
            const depth = getContainerDepth(comp);
            if (depth > maxDepth) {
              maxDepth = depth;
...
const handleQRCodePropertyChange = (
  ...
  // Modified: Updated to handle nested containers
...
const handleRemoveImage =()=>{
  ...
  // Modified: Global event listeners for drag and drop - handles nested containers
  useEffect(() => {
      ...
      if (draggingRef.current && canvasRef.current) {
        ...
        animationFrameId = requestAnimationFrame(() => {
          ...
          setTemplateData((prev: TemplateState<unknown>) => {
            ...
            // If dragging element is inside a container, calculate position relative to container hierarchy
            if (draggingRef.current?.containerId) {
              const containerChain = [];
              let currentContainerId = draggingRef.current.containerId;
              
              // Build the container chain from immediate parent to root
              while (currentContainerId) {
  ...
  // Modified: Delete handler that cascades through nested containers
  const handleDelete = (componentId: string) => {
    setTemplateData((prev: TemplateState<unknown>) => {
      const componentsToDelete = new Set<string>();
      const components = prev.barcodeState?.placedComponents || [];
      
      // Recursively find all children (including nested containers)
  ...
  // Modified: Recursive render function for nested containers
    ...
    return (
      <ResizableBox
        ...
        style={{
          ...
          zIndex: 1 + depth, // Increase z-index with depth
        }}
      >
        <div
        ...
        >
          ...
          {/* Recursively render children, including nested containers */}
  ...
  // Modified: Helper function to consolidate container children before saving - handles nesting
  const consolidateContainerChildren = () => {
    ...
    // Recursive function to consolidate a container and its children
    const consolidateContainer = (container: PlacedComponent): PlacedComponent => {
      ...
      const consolidatedChildren = children.map(child => {
        if (child.type === DesignerElementType.container) {
          // Recursively consolidate nested containers
  ...
  // Modified: Expand container children after loading - handles nesting
  const expandContainerChildren = (components: PlacedComponent[]): PlacedComponent[] => {
    const result: PlacedComponent[] = [];
    
    // Recursive function to expand containers
      ...
      >
        ...
        >
          ...
          >
            ...
            >
              {/* Modified: Render components using nested container support */}
              {templateData?.barcodeState?.placedComponents
