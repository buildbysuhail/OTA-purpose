...
interface TransactionReportCheckboxProps {
  ...
  setAllTransactions: React.Dispatch<React.SetStateAction<TransactionReportCheckbox[]>>;  
  onDataChange: (formState: {vTypes: string, drCr: string, allChecked: boolean, isDr: boolean, isCr: boolean}) => void;
  filter: any
}

const TransactionReportfilterCheckboxes: React.FC<TransactionReportCheckboxProps> = ({
  allTransactions,
  setAllTransactions,
  onDataChange,
  filter
...
useEffect(() => {
  if(allTransactions.find(x => x.checked == false))
  {
