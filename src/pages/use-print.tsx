...
export const usePrintTrans = ({ voucherType, transactionType }: VoucherType) => {
  ...
  const loadPrintData = useCallback(async (invTransMasterIDParam: number, fields: (keyof PrintCustomFields)[], detailsFields: (keyof PrintCustomFields)[], voucherTypeParam: string,
    formType: string,
    isInvTrans = false,
    isSalesView = false,
    ...
    kitchenNameParam = "",
    commonKitchenProductGroupIDParam = 0): Promise<PrintResponse> => {
    ...
    try {
      ...
      // IsBankLedger
      // ProductStockdetails
      const printData: PrintResponse = await api.getAsync(`${Urls.inv_transaction_base}${transactionType}/printData/?
