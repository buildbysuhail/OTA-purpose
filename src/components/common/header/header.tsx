...
let userSession  = useAppSelector((state: RootState) => state.UserSession);
  ...
  function menuClose() {
    ...
    if (window.innerWidth >= 992) {
      updateAppState({ ...theme, toggled: appState.toggled ? appState.toggled : '' });
    }
  }
  const toggleSidebar = () => {
    const theme =  appState;
  ...
  const updateAppStateValues = (appState: AppState): AppState => {
    return {
      ...
      dataMenuStyles: appState.dataNavLayout === 'horizontal' 
        ? appState.class === 'dark' ? 'light' : 'dark' 
        : 'dark'
    };
  };
  const ToggleDark = () => {
              ...
              <div id="dropdown-profile" className="cursor-pointer hs-dropdown-toggle ti-dropdown-toggle header-element md:!px-[0.65rem] px-2 hs-dropdown !items-center ti-dropdown [--placement:bottom-left] min-w-[150px]">
                ...
                <div
                  ...
                  <ul className="text-defaulttextcolor font-medium dark:text-[#8c9097] dark:text-white/50">
                    ...
                    <li>
                      <Link className="w-full ti-dropdown-item !text-[0.8125rem] !gap-x-0  !p-[0.65rem] !inline-flex" to="/switch-organization"><i
