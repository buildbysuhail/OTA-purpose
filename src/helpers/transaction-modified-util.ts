    ...
    return {
      transaction: {
        ...
        details: _formState.transaction.details.filter(
          (x: any) =>
            (_formState.row != undefined &&
              !isNullOrUndefinedOrZero(x.ledgerID)) ||
            (_formState.row == undefined &&
      ...
      row: {
        ...
        ledgerID:
          ...
          _formState.row?.ledgerID == 0
            ...
            : _formState.row?.ledgerID,
        ledgerName:
          _formState.row?.ledgerName == null || _formState.row?.ledgerName == ""
            ? ""
            : _formState.row?.ledgerName,
        costCentreName:
          _formState.row?.costCentreName == null ||
          _formState.row?.costCentreName == ""
            ? ""
            : _formState.row?.costCentreName,
        ...
        projectID:
          _formState.row?.projectID == undefined ||
