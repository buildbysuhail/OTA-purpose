 ...
 */
const SBForm = ({ data, defaultData, onChangeData, onChangeDefaultData, ...props }: SBTableProps) => {
  ...
  const handleChange = (id: string, value: any) => {
    try {
      ...
      console.log(`SBForm,  Error: `, error);
    }
  };
    ...
    <div className={`capitalize  max-w-[1200px] ${props.fieldClass}`}>
      {props.fields.map((field: fieldType, index) => {
        ...
        switch (fieldType) {
          case "checkbox":
            return (
              < label={field?.label} items={field?.items} field={field} data={data} defaultData={defaultData} handleChange={handleChange} />
            );
          case "slider":
            return (
            ...
            return (
              <FormControl key={`tf_${index}`}>
                <div className="flex items-center text-xs gap-10">
                  ...
                  {field?.hintAvailable && data?.pricing_type === "volume_pricing" && <SBTooltip message={field?.hintMessage} />}
                </div>
              </FormControl>
            );
          case "switch":
