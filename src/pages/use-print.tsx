...
// Complete enums matching C# exactly
...
export const initialPrintCustomFields : PrintCustomFields = {
  ...
  gt: false,
  voucherNumberGate: "",
  transactionTimeGate: new Date().toISOString(),
  totalItemsGate: 0,
  tokenBarcodeGate: "",
...
interface DtaForPrinMaster extends TransactionMaster {
  stateCode?: string;
  stateName?: string;
...
interface DataForPrinDetail extends TransactionDetail {

  abc: string;
}
interface DataForPrint {
  master: DtaForPrinMaster;
  details: DataForPrinDetail[];
  custom: PrintCustomFields;
  hasGroupHeaderPrinting: boolean;
}
export const initialDataForPrint: DataForPrint = {
  master: {
  ...
  details: [] as DataForPrinDetail[], // start empty

  custom: initialPrintCustomFields as PrintCustomFields,
  hasGroupHeaderPrinting: false
};
export const usePrintTrans = ({ voucherType, transactionType }: VoucherType) => {
  ...
  const updateDetailsAndSummary = useCallback(async (printData: DataForPrint) => {
    ...
    for (let i = 0; i < dtTransDetails.length; i++) {
      const row: DataForPrinDetail = dtTransDetails[i];
  ...
  const loadPrintData = useCallback(async (invTransMasterIDParam: number, fields: (keyof PrintCustomFields)[], voucherTypeParam: string,
    ...
    kitchenNameParam = "",
    commonKitchenProductGroupIDParam = 0): Promise<DataForPrint> => {
    ...
    let returnData: DataForPrint = initialDataForPrint;
    try {
      ...
      // IsBankLedger
      // ProductStockdetails
      const printData: DataForPrint = await api.getAsync(`${Urls.inv_transaction_base}${transactionType}/printData/?
