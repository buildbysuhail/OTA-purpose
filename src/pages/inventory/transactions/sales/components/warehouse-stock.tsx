...
const WareHouseStock: React.FC<WStockListProps> = ({
  closeModal,
  t,
  productName,
  productBatchID,
    ...
    const loadWarehouseStock = async () => {
      try {
        const res = await api.getAsync(
          `${Urls.inv_transaction_base}${formState.transactionType}/WareHouseStock/${productBatchID}`
        );
  ...
  return (
    <>
      <div className="flex items-end gap-2">
        <div className="w-full flex items-center text-lg text-primary justify-center font-bold">
          {productName || t("no_product_selected")}
        </div>
      </div>
      <div className="mt-4">
        <ErpDevGrid
