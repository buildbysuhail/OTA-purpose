...
} from "../components/types/dev-grid-column";
...
export function applyGridColumnPreferences(
  columns: DevGridColumn[],
  preferences?: GridPreference
): DevGridColumn[] {
  ...
  updatedColumns.sort((a, b) => {
    const orderA =
      preferences?.columnPreferences?.findIndex(
        (p) => p.dataField === a.dataField
      ) || 0;
    const orderB =
      preferences?.columnPreferences?.findIndex(
        (p) => p.dataField === b.dataField
      ) || 0;
...
export function getDefaultColumnPreference(
...
export const getInitialPreference = async (
  ...
  let parsedPreferences: GridPreference | undefined;
  if (
  ...
  if (parsedPreferences) {
    parsedPreferences.columnPreferences =
      parsedPreferences.columnPreferences.map((pref, index) => {
        return {
          ...pref,
          order: index + 1,
        };
    ...
    columns?.forEach((column: any, index: any) => {
      ...
      let colPreference = parsedPreferences.columnPreferences.find(
        (x) => x.dataField == column.dataField
      );
      colPreference =
        colPreference == undefined
          ? ({
              ...(getDefaultColumnPreference(column, index) as any),
              order: outIdex + index,
            } as ColumnPreference)
          : colPreference;
      mergedPreferences.push({
        ...colPreference,
        dataField:
          column.dataField ?? removeSpacesAndCapitalize(column.caption ?? ""),
      });
      // }
    });
