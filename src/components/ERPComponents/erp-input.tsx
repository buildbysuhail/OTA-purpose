...
const ERPInput = forwardRef<HTMLInputElement, ERPInputProps>(
  (
    ...
    const [borderStyles, setBorderStyles] = useState<string>(
      appState.mode == "dark"
        ? isFocused == true || isHovered == true
          ? "#ffffff"
          : "#ffffff1a"
        : `${isFocused || isHovered ? `rgb(${inputBoxState?.borderFocus})` : `rgb(${inputBoxState?.borderColor})`} `
    );
            ...
            mui: {
              ...
              "& .MuiInputLabel-root": {
                ...
                color: appState?.mode === 'dark' ? 'rgb(225,224,224)' :
                  (inputBoxState?.labelColor ? `rgb(${inputBoxState?.labelColor})` : 'rgb(84,84,84)'),
                transform:
                  _variant === "filled"
    ...
    if (_useMUI == undefined || _useMUI == false) {
      return (
        <div
          className={`${className} ${labelDirection === "vertical" ? "flex flex-col space-y-1" : "flex items-center space-x-2"}`}
          style={{
            marginBottom: `${inputBoxState?.marginBottom ?? 0}px`,
            marginTop: `${inputBoxState?.marginTop ?? 0}px`,
          }}
            ...
            <div className="flex justify-between">
              <label
                ...
                style={{
                  ...
                  color: appState?.mode === "dark" ? "rgb(225,224,224)" :
                    (inputBoxState?.labelColor ? `rgb(${inputBoxState?.labelColor})` : "rgb(84,84,84)"),
                  transform: _customSize === "customize" ? `translate(${inputBoxState?.adjustA ?? 10}px, ${inputBoxState?.adjustB ?? 10}px) scale(1)` : ``,
                }}
              >
              ...
              >
                {labelInfo && cloneElement(labelInfo, labelInfoProps ? { labelInfoProps: labelInfoProps } : {})}
              </label>
            </div>
          )}
