...
const getColumnValue = (detail: TransactionDetail, columnName: keyof TransactionDetail): number | null => {
  try {
...
export const calculateSummary = (details: any,
  formState: TransactionFormState,
   commonParams: CommonParams) => {
   let {
    useNumberFormat,
   ...
   const summaryConfig = formState.summaryConfig
    ...
    summaryConfig.forEach((config, index) => {
      try {
        ...
        const columnKey = (config.showInColumn || config.column) as keyof SummaryItems;
        if((config.showInColumn || config.column) == "vatAmount") {
          (result.summary as any)[columnKey] = round(calculatedValue,undefined, true);
        } else {
          (result.summary as any)[columnKey] = round(calculatedValue);
        }

      } catch (configError) {
        console.error(`Error processing summary config for column ${config.column}:`, configError);
      }
    });
...
export const refactorDetails = (details: any[], vType: string,
   formType: string, 
   commonParams: CommonParams,
  formState: TransactionFormState,
applicationSettings: ApplicationSettingsType) => {
  ...
  const {round, getFormattedValueIgnoreRounding} = useNumberFormat();
    ...
    for (let i = 0; i < detailsLength; i++) {
     ...
     detail.unitPrice = getFormattedValueIgnoreRounding(Number(row.unitPrice || 0));
     
     // Special handling for GRN type
     if (vType === "GRN") {
       detail.unitPriceTag = getFormattedValueIgnoreRounding(Number(row.unitPrice || 0));
     }
     
     // Cost calculations
     const costPerItem = Number(row.costPerItem || 0);
     const additionalExpense = Number(row.additionalExpense || 0);
     detail.cost = getFormattedValueIgnoreRounding(costPerItem);
     ...
     detail.totalAddExpense = Number(row.quantity || 0) * Number(row.additionalExpense || 0);
     detail.location = row.location;
     detail.profit = row.totalProfit;
     
     // Sales pricing
     detail.salesPrice = getFormattedValueIgnoreRounding(Number(row.stdSalesPrice || 0));
     detail.ratePlusTax = getFormattedValueIgnoreRounding(Number(row.rateWithTax || 0));
     ...
     detail.gross = getFormattedValueIgnoreRounding(Number(row.grossValue || 0));
     detail.discPerc = getFormattedValueIgnoreRounding(Number(row.discountPer1 || 0));
     detail.discount = getFormattedValueIgnoreRounding(Number(row.discountAmt1 || 0));
     detail.schemeDiscount = getFormattedValueIgnoreRounding(Number(row.schemeDiscAmt || 0));
     
     // VAT handling based on form type
     if (formType === "VAT") {
       detail.vatPerc = row.vatPercentage;
       detail.vatAmount = getFormattedValueIgnoreRounding(Number(row.totalVatAmount || 0));
     } else {
       detail.vatPerc = 0;
       detail.vatAmount = 0;
     }
     
     detail.netValue = getFormattedValueIgnoreRounding(Number(row.netValue || 0));
     detail.total = getFormattedValueIgnoreRounding(Number(row.netAmount || 0));
     ...
     const res  = calculateRowAmount(detail,"slNo", formState,applicationSettings,{
      ...commonParams, result: {
        transaction:{
          details:[
            {...detail}
     ...
     detail = res.transaction?.details;
...
export const attachDetails = (
  ...
  for (let i = 0; i < details.length; i++) {
    ...
    if (!detail.product || detail.product === "") {
      break;
    }
    ...
    if (Math.abs(outputRow.grossValue - outputRow.quantity * outputRow.unitPrice) > 1) {
      hasError = true;
      errors.push(`Row ${rowNumber}, Gross Value Column: Gross value calculation mismatch`);
    }
    ...
    if (applicationSettings?.productsSettings?.enableMultiWarehouseBilling && 
      ...
      try {
        ...
        hasError = true;
        errors.push(`Row ${rowNumber}, Warehouse Column: Error setting warehouse from detail - ${error}`);
      }
    }
    ...
    // Supplier reference
    outputRow.supplierProductReferenceCode = detail.supplierReferenceProductCode;
    ...
    if (applicationSettings?.inventorySettings?.carryForwardPurchaseOrderQtyToPurchase) {
      ...
      try {
        ...
        hasError = true;
        errors.push(`Row ${rowNumber}, PO Detail Quantities Column: Error setting PO detail quantities - ${error}`);
      }
    }
    
    // Memo and random key
    outputRow.memo = detail.memo;
    outputRow.randomKey = Math.random();
 ...
 export const attachMaster = (formState: TransactionFormState) => {
