          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-x-6 gap-y-3">
            {favoriteItems?.length > 0 &&
              favoriteItems?.map((favoriteItem: any) => (
                <div key={`${favoriteItem.id}-${favoriteItem.id}`} className="flex items-center min-w-0">
                  <svg
...
//   );
// };
const SearchResultBar = ({ isOpen, searchResults, selectedIndex, onItemClick }: any) => {
  const { t } = useTranslation('main');
  const navigate = useNavigate();
  return (
      ...
      <div className="dark:bg-dark-bg flex flex-col">
        {searchResults?.length > 0 ? (
          searchResults?.map((item: any, idx: number) => (
            <div className="w-full p-1" key={`SR_${idx}`}>
              <p className={`text-[13px] rounded-lg p-2 cursor-pointer ${idx === selectedIndex ? "bg-primary text-white" : "hover:bg-primary hover:text-white"}`}
                onClick={() => onItemClick(item)}>
                {t(item.title as any)}
              </p>
            </div>
