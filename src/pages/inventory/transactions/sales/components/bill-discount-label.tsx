...
const BillDiscountLabel: React.FC<GrandTotalProps> = ({ formState, t }) => {
  ...
  return (
    ...
    <div className="flex items-center">
      ...
      <span className={`text-sm font-semibold dark:text-dark-text text-gray-900 flex-1 ${isRtl ? "text-left" : "text-right"}`}>{getFormattedValue(formState.transaction.master.billDiscount ?? 0)}</span>
    </div>
  );
};
